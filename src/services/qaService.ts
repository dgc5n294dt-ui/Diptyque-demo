...
async function buildDeepSeekResponse(question: string): Promise<AskResponse> {
  ...
  if (!apiKey) {
    return {
      ...fallback,
      warnings: [...fallback.warnings, "ANSWER_PROVIDER=deepseek，但当前没有配置 DEEPSEEK_API_KEY，已回退为 mock provider。"],
      provider: "mock",
    };
  }
  ...
  try {
    ...
    return {
      ...fallback,
      warnings: [...fallback.warnings, `DeepSeek provider 调用失败，已回退为 mock provider：${error instanceof Error ? error.message : String(error)}`],
      provider: "mock",
    };
  }
}
